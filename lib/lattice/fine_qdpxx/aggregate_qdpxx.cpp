...
namespace MG {
    ...
    void DslashDirQDPXX(LatticeFermion &out, const multi1d<LatticeColorMatrix> &u,
                        const LatticeFermion &in, int dir) {
        switch (dir) {
        ...
        case 7: // Dir 3, Backward
            ...
            break;
        default: MasterLog(ERROR, "Unknown direction. You oughtnt call this\n"); QDP_abort(1);
        }
    }
    ...
    // In the full blown version, instead of the 'site' we will sum over sites in
    // a block
    void caxpyAggrQDPXX(const std::complex<double> &alpha, const LatticeFermion &x,
                        LatticeFermion &y, int site, int aggr) {
        for (int spin = aggr * (Ns / 2); spin < (aggr + 1) * (Ns / 2); ++spin) {
            for (int color = 0; color < Nc; ++color) {
    ...
    double norm2AggrQDPXX(const LatticeFermion &v, int site, int aggr) {
        ...
        for (int spin = aggr * (Ns / 2); spin < (aggr + 1) * (Ns / 2); ++spin) {
            for (int color = 0; color < Nc; ++color) {
                norm2 += v.elem(site).elem(spin).elem(color).real() *
                         v.elem(site).elem(spin).elem(color).real();
                norm2 += v.elem(site).elem(spin).elem(color).imag() *
                         v.elem(site).elem(spin).elem(color).imag();
            }
        }
    ...
    // In the full blown version, instead of the 'site' we will sum over sites in
    // a block
    std::complex<double> innerProductAggrQDPXX(const LatticeFermion &left,
                                               const LatticeFermion &right, int site, int aggr) {
                ...
                for (int curr_vec = 0; curr_vec < vecs.size(); curr_vec++) {
                    ...
                    for (int prev_vec = 0; prev_vec < curr_vec; prev_vec++) {
                        ...
                        std::complex<double> iprod =
                            innerProductAggrQDPXX(vecs[prev_vec], vecs[curr_vec], site, aggr);
                    ...
                    double inv_norm =
                        ((double)1) / sqrt(norm2AggrQDPXX(vecs[curr_vec], site, aggr));
    ...
    void dslashTripleProduct12x12SiteDirQDPXX(int dir, const multi1d<LatticeColorMatrix> &u,
                                              const LatticePropagator &in_prop,
                                              LatticePropagator &out_prop) {
    ...
    //  Initially, prop_in should be just I_{12x12}
    //  However, any unitary matrix on the sites (orthonormal basis) would do
    void dslashTripleProductSiteDirQDPXX(int dir, const multi1d<LatticeColorMatrix> &u,
        ...
        for (int j = 0; j < Ncolorspin_c; ++j) { out_vecs[j] = zero; }
                        ...
                        for (int matmul_row = 0; matmul_row < Ncolor_c; ++matmul_row) {
                            for (int matmul_col = 0; matmul_col < Ncolor_c; ++matmul_col) {
                                ...
                                for (int k = 0; k < Ncolor_c; ++k) {
                                    ...
                                    // Right vector
                                    REAL right_r =
                                        out_vecs[col].elem(site).elem(spin).elem(color).real();
                                    REAL right_i =
                                        out_vecs[col].elem(site).elem(spin).elem(color).imag();
                                    ...
                                    REAL left_r = in_vecs[matmul_row]
                                                      .elem(site)
                                                      .elem(spin)
                                                      .elem(color)
                                                      .real();
                                    REAL left_i = in_vecs[matmul_row]
                                                      .elem(site)
                                                      .elem(spin)
                                                      .elem(color)
                                                      .imag();
    ...
    void clovTripleProductSiteQDPXX(const QDPCloverTerm &clov,
        ...
        // Zero the output
        for (int j = 0; j < Ncolor_c; ++j) { out_vecs[j] = zero; }
        ...
        for (int j = 0; j < Ncolor_c; ++j) {
            ...
            LatticeFermion tmp = zero;
            for (int cb = 0; cb < 2; ++cb) { clov.apply(out_vecs[j], in_vecs[j], 0, cb); }
        }
                    ...
                    for (int matmul_row = 0; matmul_row < Ncolor_c; ++matmul_row) {
                        for (int matmul_col = 0; matmul_col < Ncolor_c; ++matmul_col) {
                            ...
                            int coarse_clov_index =
                                n_complex *
                                (matmul_row + c_offset + Ncolorspin_c * (matmul_col + c_offset));
                            ...
                            for (int k = 0; k < Ncolor_c; ++k) {
                                ...
                                int spin =
                                    k / Nc +
                                    chiral * (Ns / 2); // Upper or lower spin depending on chiral
                                ...
                                REAL right_r =
                                    out_vecs[matmul_col].elem(site).elem(spin).elem(color).real();
                                REAL right_i =
                                    out_vecs[matmul_col].elem(site).elem(spin).elem(color).imag();
                                ...
                                // so index with row % Ncolor_c = matmul_row
                                REAL left_r =
                                    in_vecs[matmul_row].elem(site).elem(spin).elem(color).real();
                                REAL left_i =
                                    in_vecs[matmul_row].elem(site).elem(spin).elem(color).imag();

                                // Accumulate inner product V^H_row A_column
                                coarse_clov[RE + coarse_clov_index] +=
                                    left_r * right_r + left_i * right_i;
                                coarse_clov[IM + coarse_clov_index] +=
                                    left_r * right_i - right_r * left_i;
                            }
                        }
                    }
    ...
    //
    void restrictSpinorQDPXXFineToCoarse(const multi1d<LatticeFermion> &v,
                    ...
                    for (int coarse_color = 0; coarse_color < num_coarse_color; coarse_color++) {
                        ...
                        for (int spin = 0; spin < Ns / 2; ++spin) {
                            for (int color = 0; color < Nc; ++color) {
                                ...
                                int targ_spin =
                                    spin + chiral * (Ns / 2); // Offset by whether upper/lower

                                REAL left_r = v[coarse_color]
                                                  .elem(qdpsite)
                                                  .elem(targ_spin)
                                                  .elem(color)
                                                  .real();
                                REAL left_i = v[coarse_color]
                                                  .elem(qdpsite)
                                                  .elem(targ_spin)
                                                  .elem(color)
                                                  .imag();
                                ...
                                REAL right_r =
                                    ferm_in.elem(qdpsite).elem(targ_spin).elem(color).real();
                                REAL right_i =
                                    ferm_in.elem(qdpsite).elem(targ_spin).elem(color).imag();

                                // It is V_j^H  ferm_in so conj(left)*right.
                                site_spinor[RE + n_complex * coarse_colorspin] +=
                                    left_r * right_r + left_i * right_i;
                                site_spinor[IM + n_complex * coarse_colorspin] +=
                                    left_r * right_i - right_r * left_i;
                            }
                        }
                    }
    ...
    void prolongateSpinorCoarseToQDPXXFine(const multi1d<LatticeFermion> &v,
                ...
                for (int fine_spin = 0; fine_spin < Ns; ++fine_spin) {
                    ...
                    for (int fine_color = 0; fine_color < Nc; fine_color++) {
                        ...
                        for (int coarse_color = 0; coarse_color < num_coarse_color;
                            ...
                            fine_out.elem(qdpsite).elem(fine_spin).elem(fine_color).real() +=
                                left_r * right_r - left_i * right_i;
                            fine_out.elem(qdpsite).elem(fine_spin).elem(fine_color).imag() +=
                                left_i * right_r + left_r * right_i;
                        }
                    }
                }
            }
