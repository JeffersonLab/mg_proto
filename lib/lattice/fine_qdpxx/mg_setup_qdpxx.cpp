						...
						MGLevelCoarseEO& coarse_level)
{
	...
	fine_level.null_solver = std::make_shared<UnprecBiCGStabSolverCoarseWrapper>(std::make_shared<BiCGStabSolverCoarse>(*M_fine, params)
			,M_fine);
	...
	coarse_level.M = std::make_shared<const CoarseEOWilsonCloverLinearOperator>(coarse_level.gauge,fine_level_id+1);
