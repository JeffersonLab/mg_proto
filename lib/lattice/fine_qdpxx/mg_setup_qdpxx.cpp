...
namespace MG {
    ...
    void SetupQDPXXToCoarseGenerateVecs(const SetupParams &p,
        ...
        for (int k = 0; k < num_vecs; ++k) { gaussian(fine_level.null_vecs[k]); }

        for (int k = 0; k < num_vecs; ++k) {
            std::vector<LinearSolverResults> res =
                (*(fine_level.null_solver))(fine_level.null_vecs[k], b, ABSOLUTE);
            assert(res.size() == 1);
            QDPIO::cout << "BiCGStab Solver Took: " << res[0].n_count << " iterations" << std::endl;
        }
    }
    ...
    void SetupQDPXXToCoarseVecsIn(const SetupParams &p,
                                  ...
                                  MGLevelQDPXX &fine_level, MGLevelCoarse &coarse_level) {
        // For sake of form
        IndexArray latdims = {{QDP::Layout::subgridLattSize()[0], QDP::Layout::subgridLattSize()[1],
        ...
        if (!fine_level.null_solver) {
            fine_level.null_solver =
                std::make_shared<BiCGStabSolverQDPXX>(*M_fine, p.null_solver_params[0]);
        }

        int num_vecs = fine_level.null_vecs.size();
        if (num_vecs != p.n_vecs[0]) {
            MasterLog(ERROR, "QDPXX Setup is called without initing vectors, but initial vectors "
                             "are not initialized");
        }
        ...
        orthonormalizeBlockAggregatesQDPXX(fine_level.null_vecs, fine_level.blocklist);
