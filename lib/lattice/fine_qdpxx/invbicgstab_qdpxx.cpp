...
namespace MG {
...

    {
        ...
        if (resid_type == RELATIVE) { rsd_sq *= chi_sq; }
        ...
        if (VerboseP) {
            MasterLog(INFO, "BiCGStab: level=%d iter=0 || r ||^2=%16.8e Target || r ||^2=%16.8e",
                      level, toDouble(r_norm), toDouble(rsd_sq));
        }
        ...
        for (int k = 1; k <= MaxIter && !convP; k++) {
            ...
            if (VerboseP) {
                MasterLog(INFO,
                          "BiCGStab: level=%d iter=%d || r ||^2=%16.8e Target || r ||^2=%16.8e",
                          level, k, toDouble(r_norm), toDouble(rsd_sq));
            }
