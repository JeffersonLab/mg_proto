...
#pragma omp parallel for
    ...
    void caxpyBlockAggrQDPXX(const std::complex<double> &alpha, const LatticeFermion &x,
                             LatticeFermion &y, const Block &block, int aggr) {
        auto block_sitelist = block.getCBSiteList();
        int num_sites = block.getNumSites();
...
#pragma omp parallel for reduction(+ : block_sum)
    ...
    std::complex<double> innerProductBlockAggrQDPXX(const LatticeFermion &left,
                                                    const LatticeFermion &right, const Block &block,
                                                    int aggr) {
        auto block_sitelist = block.getCBSiteList();
        int num_sites = block.getNumSites();
        double real_part = 0;
...
#pragma omp parallel for reduction(+ : real_part) reduction(+ : imag_part)
    ...
    void extractAggregateQDPXX(LatticeFermion &target, const LatticeFermion &src,
...
#pragma omp parallel for
        for (int site = 0; site < num_sites; ++site) {
            ...
            for (int spin = aggr * Ns / 2; spin < (aggr + 1) * Ns / 2; ++spin) {
                for (int color = 0; color < 3; ++color) {
                    target.elem(qdpsite).elem(spin).elem(color).real() =
                        src.elem(qdpsite).elem(spin).elem(color).real();
                    target.elem(qdpsite).elem(spin).elem(color).imag() =
                        src.elem(qdpsite).elem(spin).elem(color).imag();
                }
            }
        }
    }
    ...
    void orthonormalizeBlockAggregatesQDPXX(multi1d<LatticeFermion> &vecs,
                ...
                for (int curr_vec = 0; curr_vec < vecs.size(); curr_vec++) {
                    ...
                    for (int prev_vec = 0; prev_vec < curr_vec; prev_vec++) {
                        ...
                        std::complex<double> iprod =
                            innerProductBlockAggrQDPXX(vecs[prev_vec], vecs[curr_vec], block, aggr);
                        ...
                        caxpyBlockAggrQDPXX(minus_iprod, vecs[prev_vec], vecs[curr_vec], block,
                                            aggr);
                    }

                    // Normalize current vector
                    double inv_norm =
                        ((double)1) / sqrt(norm2BlockAggrQDPXX(vecs[curr_vec], block, aggr));
                        ...
                        for (int chiral = 0; chiral < 2; ++chiral) {
                            ...
                            for (int spin = 0; spin < Ns / 2; ++spin) {
                                for (int color = 0; color < Nc; ++color) {
                                    ...
                                    REAL right_r =
                                        ferm_in.elem(fine_site).elem(targ_spin).elem(color).real();
                                    REAL right_i =
                                        ferm_in.elem(fine_site).elem(targ_spin).elem(color).imag();

                                    // It is V_j^H  ferm_in so conj(left)*right.
                                    coarse_site_spinor[RE + n_complex * coarse_colorspin] +=
                                        left_r * right_r + left_i * right_i;
                                    coarse_site_spinor[IM + n_complex * coarse_colorspin] +=
    ...
    void dslashTripleProductDirQDPXX(const std::vector<Block> &blocklist, int dir,
        ...
        for (int j = 0; j < num_coarse_colorspin; ++j) { out_vecs[j] = zero; }
			...
			QDPIO::cout <<  " coarse_coords=(" << coarse_coords[0]
                ...
                //  ---------------------------------------------
                {
                    ...
                    for (IndexType fine_site_idx = 0;
                        ...
                        multi1d<int> qdp_coords =
                            Layout::siteCoords(Layout::nodeNumber(), qdp_site);
                                ...
                                for (int matmul_col = 0; matmul_col < num_coarse_colors;
                                    ...
                                    for (int matmul_row = 0; matmul_row < num_coarse_colors;
                                        ...
                                        for (int k = 0; k < num_spincolor_per_chiral; ++k) {
                                            ...
                                            // Right vector
                                            float right_r = (float)(out_vecs[col]
                                                                        .elem(qdp_site)
                                                                        .elem(spin)
                                                                        .elem(color)
                                                                        .real());
                                            float right_i = (float)(out_vecs[col]
                                                                        .elem(qdp_site)
                                                                        .elem(spin)
                                                                        .elem(color)
                                                                        .imag());
                                            ...
                                            float left_r = (float)(in_vecs[matmul_row]
                                                                       .elem(qdp_site)
                                                                       .elem(spin)
                                                                       .elem(color)
                                                                       .real());
                                            float left_i = (float)(in_vecs[matmul_row]
                                                                       .elem(qdp_site)
                                                                       .elem(spin)
                                                                       .elem(color)
                                                                       .imag());
    ...
    void clovTripleProductQDPXX(const std::vector<Block> &blocklist, const QDPCloverTerm &clov,
                                const multi1d<LatticeFermion> &in_vecs, CoarseGauge &gauge_clover) {
        ...
        // Zero the output
        for (int j = 0; j < num_coarse_colors; ++j) { out_vecs[j] = zero; }
                        ...
                        for (int matmul_col = 0; matmul_col < num_coarse_colors; ++matmul_col) {
                            ...
                            for (int matmul_row = 0; matmul_row < num_coarse_colors; ++matmul_row) {
                                ...
                                int coarse_clov_index =
                                    n_complex * ((matmul_row + num_coarse_colors * matmul_col) +
                                ...
                                for (int k = 0; k < num_spincolor_per_chiral; ++k) {
                                    ...
                                    // Accumulate inner product V^H_row A_column
                                    tmp_link[RE + coarse_clov_index] +=
                                        (double)(left_r * right_r + left_i * right_i);
                                    tmp_link[IM + coarse_clov_index] +=
                        ...
                        for (int row = 0; row < num_coarse_colors; ++row) {
                            ...
                            coarse_clov[RE +
                                ...
                                tmp_link[RE + n_complex * (row + num_coarse_colors * col) +
                                         chiral * n_complex * num_coarse_colors *
                                             num_coarse_colors];
