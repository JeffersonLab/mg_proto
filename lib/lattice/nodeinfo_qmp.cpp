                ...
                // Find the coordinates and node_id's of the neighbors
                {
                    ...
                    for (IndexType mu = 0; mu < n_dim; ++mu) {
                        ...
                        for (auto dir = 0; dir <= 1; ++dir) {
                            ...
                            if (dir == MG_FORWARD) {
                                neigh_coords[mu]++;
                                if (neigh_coords[mu] >= static_cast<int>(_node_dims[mu]))
                                    neigh_coords[mu] = 0;
                            }

                            _neighbor_ids[mu][dir] =
                                static_cast<IndexType>(QMP_get_node_number_from(neigh_coords));
                        }
                    }
                }
