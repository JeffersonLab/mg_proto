...
#    include <cblas.h>
    ...
    void XGEMM(const char *transa, const char *transb, LAPACK_BLASINT m, LAPACK_BLASINT n,
               LAPACK_BLASINT k, std::complex<float> alpha, const std::complex<float> *a,
               LAPACK_BLASINT lda, const std::complex<float> *b, LAPACK_BLASINT ldb,
               std::complex<float> beta, std::complex<float> *c, LAPACK_BLASINT ldc) {
        assert(c != a && c != b);
        if (m == 0 || n == 0) return;
        if (k == 0) {
            for (int j = 0; j < n; j++)
                for (int i = 0; i < m; i++) c[i + ldc * j] *= beta;
            return;
        }
#ifndef MGPROTO_USE_CBLAS
        cgemm_(transa, transb, &m, &n, &k, &alpha, a, &lda, b, &ldb, &beta, c, &ldc);
#else
        cblas_cgemm(CblasColMajor, toTrans(transa), toTrans(transb), m, n, k, &alpha, a, lda, b,
                    ldb, &beta, c, ldc);
#endif
    }

    void CMatMultNaive(float *y, const float *A, const float *x, IndexType N, IndexType ncol) {
        CMatMultCoeffAddNaive(0.0, y, 1.0, A, x, N, ncol);
    }

    void CMatMultAddNaive(float *y, const float *A, const float *x, IndexType N, IndexType ncol) {
        CMatMultCoeffAddNaive(1.0, y, 1.0, A, x, N, ncol);
    }

    void CMatMultCoeffAddNaive(float beta, std::complex<float> *y, IndexType ldy, float alpha,
                               const std::complex<float> *A, IndexType ldA,
                               const std::complex<float> *x, IndexType ldx, IndexType Arows,
                               IndexType Acols, IndexType xcols) {
        if (fabs(beta) == 0.0) {
    ...
    void CMatMultCoeffAddNaive(float beta, float *y, float alpha, const float *A, const float *x,
                               IndexType N, IndexType ncol) {
        if (fabs(beta) == 0.0)
    ...
    void CMatAdjMultNaive(float *y, const float *A, const float *x, IndexType N, IndexType ncol) {
        for (IndexType i = 0; i < N * ncol * 2; ++i) y[i] = 0.0;
    ...
    void CMatAdjMultCoeffAddNaive(float beta, std::complex<float> *y, IndexType ldy, float alpha,
                                  const std::complex<float> *A, IndexType ldA,
                                  const std::complex<float> *x, IndexType ldx, IndexType Arows,
                                  IndexType Acols, IndexType xcols) {
        if (fabs(beta) == 0.0) {
    ...
    void GcCMatMultGcNaive(float *y, const float *A, const float *x, IndexType N, IndexType ncol) {
        GcCMatMultGcCoeffAddNaive(0.0, y, 1.0, A, x, N, ncol);
    }

    void GcCMatMultGcCoeffAddNaive(float beta, float *y, float alpha, const float *A,
                                   const float *x, IndexType N, IndexType ncol) {
        if (fabs(beta) == 0.0)
            for (IndexType i = 0; i < N * ncol * 2; ++i) y[i] = 0.0;
        ...
        XGEMM("N", "N", N / 2, ncol, N / 2, alpha, &Ac[N * N / 2 + N / 2], N, &xc[N / 2], N, 1.0,
              &yc[N / 2], N);
    }
}
