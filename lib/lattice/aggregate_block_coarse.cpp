...
#pragma omp parallel for
    ...
    void caxpyBlockAggr(const std::complex<double> &alpha, const CoarseSpinor &x, CoarseSpinor &y,
...
#pragma omp parallel for
        for (int site = 0; site < num_sites; ++site) {
            ...
            float *y_site_data = y.GetSiteDataPtr(0, cbsite.cb, cbsite.site);
...
#pragma omp parallel for reduction(+ : block_sum)
    ...
    std::complex<double> innerProductBlockAggr(const CoarseSpinor &left, const CoarseSpinor &right,
                                               const Block &block, int aggr) {
...
#pragma omp parallel for reduction(+ : real_part) reduction(+ : imag_part)
        for (int site = 0; site < num_sites; ++site) {
            ...
            const CBSite &cbsite = block_sitelist[site];
            const float *left_site_data = left.GetSiteDataPtr(0, cbsite.cb, cbsite.site);
            const float *right_site_data = right.GetSiteDataPtr(0, cbsite.cb, cbsite.site);
            for (int colorspin = min_cspin; colorspin < max_cspin; ++colorspin) {
    ...
    void extractAggregateBlock(CoarseSpinor &target, const CoarseSpinor &src, const Block &block,
                               int aggr) {
...
#pragma omp parallel for
        for (int site = 0; site < num_sites; ++site) {
            ...
            for (int colorspin = min_cspin; colorspin < max_cspin; ++colorspin) {
                target_site_data[RE + n_complex * colorspin] =
                    src_site_data[RE + n_complex * colorspin];
                target_site_data[IM + n_complex * colorspin] =
                    src_site_data[IM + n_complex * colorspin];
            }
        }
    }
...
#pragma omp parallel for collapse(2)
        for (int cb = 0; cb < n_checkerboard; ++cb) {
            for (int site = 0; site < num_cbsites; ++site) {
                ...
                const float *src_site_data = src.GetSiteDataPtr(0, cb, site);
                float *target_site_data = target.GetSiteDataPtr(0, cb, site);
                for (int colorspin = min_cspin; colorspin < max_cspin; ++colorspin) {
                    target_site_data[RE + n_complex * colorspin] =
                        src_site_data[RE + n_complex * colorspin];
                    target_site_data[IM + n_complex * colorspin] =
...
#pragma omp parallel for collapse(2)
    ...
    void restrictSpinor(const std::vector<Block> &blocklist,
                        const std::vector<std::shared_ptr<CoarseSpinor>> &fine_vecs,
        ...
        if (n_checkerboard * num_coarse_cbsites != static_cast<IndexType>(blocklist.size())) {
            MasterLog(
                ERROR,
                "restrictSpinor error: n_checkerboard=%d num_coarse_cbsites=%d blocklist_size=%d",
        ...
        assert(fine_in.GetNCol() == coarse_out.GetNCol());
                    ...
                    for (int coarse_color = 0; coarse_color < num_coarse_colors; coarse_color++) {
                        for (int col = 0; col < ncol; ++col) {
                            ...
                            float *coarse_site_spinor =
                                coarse_out.GetSiteDataPtr(col, block_cb, block_cbsite);
                            ...
                            for (IndexType fine_site_idx = 0;
                                ...
                                const float *ferm_in_site_data =
                                    fine_in.GetSiteDataPtr(col, fine_site.cb, fine_site.site);
                                const float *vec_in_site_data =
                                    fine_vecs[coarse_color]->GetSiteDataPtr(0, fine_site.cb,
                                                                            fine_site.site);
...
#pragma omp simd reduction(+ : iprod_re, iprod_im)
                                for (int fine_colorspin = min_fine_cspin;
                                    ...
                                    float left_r =
                                        vec_in_site_data[RE + n_complex * fine_colorspin];
                                    float left_i =
                                        vec_in_site_data[IM + n_complex * fine_colorspin];
                                    ...
                                    float right_r =
                                        ferm_in_site_data[RE + n_complex * fine_colorspin];
                                    float right_i =
                                        ferm_in_site_data[IM + n_complex * fine_colorspin];
                    ...
                    for (int fine_color = 0; fine_color < n_fine_per_chiral; fine_color++) {
                        for (int col = 0; col < ncol; ++col) {
                            ...
                            for (unsigned int fine_site_idx = 0; fine_site_idx < num_fine_sitelist;
                                ...
                                float *fine_site_data =
                                    fine_out.GetSiteDataPtr(col, cbsite.cb, cbsite.site);
...
#pragma omp simd reduction(+ : csum_re, csum_im)
                                for (int coarse_color = 0; coarse_color < num_coarse_colors;
                                    ...
                                    const float *vec_in_data = fine_v[coarse_color]->GetSiteDataPtr(
                                        0, cbsite.cb, cbsite.site);

                                    int coarse_colorspin =
                                        coarse_color +
                                        chiral *
                                            num_coarse_colors; // coarse_color(chiral) is the row

                                    float left_r = vec_in_data
                                        [RE +
                                         n_complex *
                                             fine_colorspin]; // Fine_color(chiral) is row, coarse_color is column
                                    float left_i = vec_in_data[IM + n_complex * fine_colorspin];

                                    float right_r =
                                        coarse_spinor[RE + n_complex * coarse_colorspin];
                                    float right_i =
                                        coarse_spinor[IM + n_complex * coarse_colorspin];
    ...
    void dslashTripleProductDir(const CoarseDiracOp &D_op, const std::vector<Block> &blocklist,
                                int dir, const CoarseGauge &u,
        ...
        for (IndexType coarse_cb = 0; coarse_cb < n_checkerboard; ++coarse_cb) {
            for (IndexType coarse_cbsite = 0; coarse_cbsite < num_coarse_cbsites; ++coarse_cbsite) {
                ...
                //  ---------------------------------------------
                {
                    ...
                    std::vector<double> tmp_link(n_complex * num_coarse_colorspin *
                                                 num_coarse_colorspin);
                    for (int j = 0; j < n_complex * num_coarse_colorspin * num_coarse_colorspin;
                         ++j) {
                        tmp_link[j] = 0;
                    }

                    for (IndexType fine_site_idx = 0;
                         fine_site_idx < static_cast<IndexType>(num_sites); ++fine_site_idx) {
                                    ...
                                    for (int matmul_col = 0; matmul_col < num_coarse_colors;
                                        ...
                                        for (int k = 0; k < num_spincolor_per_chiral; ++k) {
                                            ...
                                            const float *right_vector_data =
                                                out_vecs[col]->GetSiteDataPtr(0, fine_cbsite.cb,
                                                                              fine_cbsite.site);
                                            const float right_r =
                                            ...
                                            const float *left_vector_data =
                                                in_vecs[matmul_row]->GetSiteDataPtr(
                                                    0, fine_cbsite.cb, fine_cbsite.site);
                                            const float left_r =
                                                left_vector_data[RE + sc * n_complex];
                                            const float left_i =
                                                left_vector_data[IM + sc * n_complex];

                                            // Accumulate inner product V^H_row A_column
                                            tmp_link[RE + coarse_link_index] +=
                                                (double)(left_r * right_r + left_i * right_i);
                                            tmp_link[IM + coarse_link_index] +=
                    ...
                    for (int row = 0; row < num_coarse_colorspin; ++row) {
                        for (int col = 0; col < num_coarse_colorspin; ++col) {
                            ...
                            coarse_link[RE + coarse_link_index] +=
                                (float)(tmp_link[RE + coarse_link_index]);
                            coarse_link[IM + coarse_link_index] +=
                                (float)(tmp_link[IM + coarse_link_index]);
...
#pragma omp parallel for collapse(2)
    ...
    template <>
    inline const float *InvDiagAccessor<CoarseGauge>::get(const CoarseGauge &in, int cb,
                                                          int cbsite) {
