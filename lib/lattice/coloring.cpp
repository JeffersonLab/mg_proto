...
Coors index2coor(const Indices &indices, const CoorType dim) {
    ...
    for (unsigned int i = 0; i < indices.size(); i++)
        for (unsigned int j = 0; j < dim.size(); j++) coors[i][j] = (indices[i] / p[j]) % dim[j];
...
Coors neighbors_upto_distance(unsigned int dist, const CoorType &dim) {
    ...
    for (unsigned int i = 0; i < dist; i++) neighbors_pattern = neighbors(neighbors_pattern, dim);
...
std::vector<unsigned int> get_motive(unsigned int dist, const CoorType &dim,
                                     unsigned int &num_colors, CBSubset subset) {
    dist = dist / 2 * 2;
...
Indices get_colors(unsigned int dist, const CoorType &dim, unsigned int &num_colors,
    ...
    for (unsigned int i = 0; i < dim.size(); i++) dim_motive[i] = (dim[i] % d == 0 ? d : dim[i]);
...
Coloring::Coloring(const std::shared_ptr<LatticeInfo> info, unsigned int distance, CBSubset subset)
    ...
    _local_colors[1].resize(_info->GetNumSites() / 2);
...
#pragma omp parallel for collapse(2) schedule(static)
    for (int cb = 0; cb < 2; ++cb) {
        for (int cbsite = 0; cbsite < num_cbsites; ++cbsite) {
            ...
            _info->LocalCoordToGlobalCoord(global_coor, local_coor);
            _local_colors[cb][cbsite] =
                colors[coor2index(Coors(1, global_coor), global_lattice_dims)[0]];
        }
    }
}
