...
SpinorHalo::SpinorHalo(const LatticeInfo& info) : _latt_info(info),
_node_info(info.GetNodeInfo())
{
	const IndexArray& latt_size = _latt_info.GetLatticeDimensions();

	_n_face_dir[X_DIR] = (latt_size[Y_DIR]*latt_size[Z_DIR]*latt_size[T_DIR]);
	_n_face_dir[Y_DIR] = (latt_size[X_DIR]*latt_size[Z_DIR]*latt_size[T_DIR]);
	_n_face_dir[Z_DIR] = (latt_size[X_DIR]*latt_size[Y_DIR]*latt_size[T_DIR]);
	_n_face_dir[T_DIR] = (latt_size[X_DIR]*latt_size[Y_DIR]*latt_size[Z_DIR]);
...
inline
int
SpinorHalo::NumNonLocalDirs() {
	return _num_nonlocal_dir;
}

void
SpinorHalo::StartSendToDir(int mu)
{
...
void
SpinorHalo::FinishSendToDir(int mu)
{
...
void
SpinorHalo::StartRecvFromDir(int mu)
{
...
void
SpinorHalo::FinishRecvFromDir(int mu)
{
