...
namespace MG {
	 ...
	 */
        ...
        std::vector<std::complex<float>> negate(const std::vector<std::complex<T>> &x) {
            ...
            std::transform(x.begin(), x.end(), r.begin(),
                           [](const std::complex<T> &f) -> std::complex<T> { return -f; });
            return r;
        }
    }
...

    {
        ...
        if (VerboseP) { MasterLog(INFO, "BiCGStab: level=%d Solver Staring: ", level); }
        ...
        for (int col = 0; col < ncol; ++col) {
            if (VerboseP) {
                MasterLog(
                    INFO,
                    "BiCGStab: col=%d level=%d iter=0 || r ||^2=%16.8e Target || r ||^2=%16.8e",
                    col, level, r_norm[col], rsd_sq[col]);
            }
            ...
            if (r_norm[col] <= rsd_sq[col]) {
                ...
                if (resid_type == ABSOLUTE) {
                    if (VerboseP) {
                        MasterLog(INFO,
                                  "BiCGStab: col=%d level=%d Final Absolute Residua: || r ||_accum "
                                  "= %16.8e  || r ||_actual = %16.8e",
                    ...
                    if (VerboseP) {
                        MasterLog(INFO,
                                  "BiCGStab: col=%d level=%d Final Relative Residua: || r ||/|| b "
        ...
        if (all(convP)) { return ret; }

        // Now initialise v = p = 0
        CoarseSpinor p(info, ncol);
        ZeroVec(p, subset);
        CoarseSpinor v(info, ncol);
        ZeroVec(v, subset);
        ...
        for (int k = 1; k <= MaxIter && !all(convP); k++) {
            ...
            for (int col = 0; col < ncol; ++col)
                beta[col] = (rho[col] / rho_prev[col]) * (alpha[col] / omega[col]);
            ...
            for (int col = 0; col < ncol; ++col) {
                if (std::abs(ctmp[col]) == 0) {
                    MasterLog(ERROR, "BiCGStab: col=%d level=%d breakdown: <r_0|v> = 0", col,
                              level);
                    convP[col] = true;
                }
            }
            ...
            for (int col = 0; col < ncol; ++col) {
                if (std::abs(t_norm[col]) == 0) {
                    MasterLog(ERROR, "BiCGStab: col=%d level=%d Breakdown || Ms || = || t || = 0",
                              col, level);
                    convP[col] = true;
                }
            }
            ...
            if (VerboseP) {
                for (int col = 0; col < ncol; ++col) {
                    MasterLog(INFO,
                              "BiCGStab: col=%d level=%d iter=%d || r ||^2=%16.8e  Target || r "
                              "||^2=%16.8e",
                              col, level, k, r_norm[col], rsd_sq[col]);
                }
            }
