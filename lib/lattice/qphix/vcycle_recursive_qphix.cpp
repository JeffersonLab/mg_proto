...
namespace MG
{
	VCycleRecursiveQPhiX::VCycleRecursiveQPhiX( const std::vector<VCycleParams>& vcycle_params,
						  const QPhiXMultigridLevels& mg_levels ) : _vcycle_params(vcycle_params), _mg_levels(mg_levels) {
		...
		_pre_smoother = std::make_shared< const MRSmootherQPhiXF >(*(_mg_levels.fine_level.M), _vcycle_params[0].pre_smoother_params);
		_post_smoother = std::make_shared< const MRSmootherQPhiXF >(*(_mg_levels.fine_level.M), _vcycle_params[0].post_smoother_params);
		MasterLog(INFO,"Creating Toplevel VCycle");
		_toplevel_vcycle = std::make_shared< const VCycleQPhiXCoarse2 >(
	...
	LinearSolverResults
	VCycleRecursiveQPhiX::operator()(QPhiXSpinor& out,
	      const QPhiXSpinor& in, ResiduumType resid_type ) const
	{
	...
	VCycleRecursiveQPhiXEO::VCycleRecursiveQPhiXEO( const std::vector<VCycleParams>& vcycle_params,
						  const QPhiXMultigridLevelsEO& mg_levels ) : _vcycle_params(vcycle_params), _mg_levels(mg_levels) {
		...
		for(int coarse_idx=n_levels-2; coarse_idx >= 0; --coarse_idx) {
			...
			if( coarse_idx == n_levels-2) {
				...
				// Bottom level There is only a bottom solver.
#if 0
				_bottom_solver[coarse_idx] = std::make_shared< const UnprecFGMRESSolverCoarseWrapper >(
							std::make_shared<const FGMRESSolverCoarse>(*this_level_linp,
										_vcycle_params[coarse_idx].bottom_solver_params,
										nullptr),
										this_level_linop);

#else
				// This becomes an unprec wrapper.
				_bottom_solver[coarse_idx] = std::make_shared<const UnprecBiCGStabSolverCoarseWrapper>(
						std::make_shared< const BiCGStabSolverCoarse >(*this_level_linop,
											_vcycle_params[coarse_idx].bottom_solver_params),
											this_level_linop);
				...
				_coarse_presmoother[coarse_idx] = std::make_shared<const UnprecMRSmootherCoarseWrapper>(std::make_shared<const MRSmootherCoarse >(*this_level_linop,
							_vcycle_params[coarse_idx+1].pre_smoother_params),this_level_linop);

				MasterLog(INFO, "Creating PreSmoother on Level %d using VCycleParams[%d]", coarse_idx+1, coarse_idx+1);
				// This becomes a wrapper
				_coarse_postsmoother[coarse_idx] = std::make_shared<const UnprecMRSmootherCoarseWrapper>(std::make_shared<const MRSmootherCoarse >(*this_level_linop,
						_vcycle_params[coarse_idx+1].post_smoother_params), this_level_linop);

				MasterLog(INFO, "Creating VCycle Between Levels: %d -> %d using VCycleParams[%d]", coarse_idx+1, coarse_idx+2,coarse_idx+1);
				_coarse_vcycle[coarse_idx] = std::make_shared< const VCycleCoarseEO >(
						(*(_mg_levels.coarse_levels[coarse_idx+1].info)),
						(_mg_levels.coarse_levels[coarse_idx].blocklist),
						(_mg_levels.coarse_levels[coarse_idx].null_vecs),
						(*(_mg_levels.coarse_levels[coarse_idx].M)),
				...
				// This becomes a wrapper
				_bottom_solver[coarse_idx] = std::make_shared<const UnprecFGMRESSolverCoarseWrapper>(
							std::make_shared<const FGMRESSolverCoarse>(*this_level_linop,
									vcycle_params[coarse_idx].bottom_solver_params,
									_coarse_vcycle[coarse_idx].get()),
								this_level_linop);
		...
		_post_smoother = std::make_shared< const MRSmootherQPhiXF >(*(_mg_levels.fine_level.M), _vcycle_params[0].post_smoother_params);
		MasterLog(INFO,"Creating Toplevel VCycle");
		_toplevel_vcycle = std::make_shared< const VCycleQPhiXCoarseEO2 >(
		    *(_mg_levels.fine_level.info), // Fine Info
		    *(_mg_levels.coarse_levels[0].info),  // Coarse info for first coarse level
		    (_mg_levels.fine_level.blocklist),   // Block List
		    (_mg_levels.fine_level.null_vecs),   // Null vecs
	...
	LinearSolverResults
	VCycleRecursiveQPhiXEO::operator()(QPhiXSpinor& out,
	      const QPhiXSpinor& in, ResiduumType resid_type ) const
	{
		LinearSolverResults ret = (*_toplevel_vcycle )( out, in, resid_type );
		return ret;
