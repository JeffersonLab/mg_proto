...
namespace MG {
    namespace MGQPhiX {
        ...
        bool IsGeomInitialized(void) { return _theGeom ? true : false; }

        //! Initialize the Geometry
        void InitializeGeom(const LatticeInfo &info) {
            const IndexArray &latdims = info.GetLatticeDimensions();
            int lattSize[4] = {latdims[0], latdims[1], latdims[2], latdims[3]};
            ...
            if (CLI.getPxyz() != 0) {
                MasterLog(INFO, "WARNING: Non-zero padding for Pxyz(=%d) is ignored.",
                          CLI.getPxyz());
            }

            _theGeom.reset(new Geom(lattSize, CLI.getBy(), CLI.getBz(), CLI.getNCores(),
                                    CLI.getSy(), CLI.getSz(),
                                    0, // enforce no padding
                                    0, // enforce no padding
                                    CLI.getMinCt(), true));

            _theGeomF.reset(new GeomF(lattSize, CLI.getBy(), CLI.getBz(), CLI.getNCores(),
                                      CLI.getSy(), CLI.getSz(),
                                      0, // enforce no padding
                                      0, // enforce no padding
                                      CLI.getMinCt(), true));
        ...
        //  otherwise barf
        template <> Geom &GetGeom<double>(void) {
            if (!_theGeom) { MasterLog(ERROR, "QPhiX Geometry is uninitialized"); }
            return *_theGeom;
        }

        //! Get back a geometry if it is initialized
        //  otherwise barf
        template <> GeomF &GetGeom<float>(void) {
            if (!_theGeom) { MasterLog(ERROR, "QPhiX Geometry is uninitialized"); }
            return *_theGeomF;
        }
    }
