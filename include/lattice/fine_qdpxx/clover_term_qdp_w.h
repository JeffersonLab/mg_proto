 ...
 *
 */
    template <typename T, typename U> class QDPCloverTermT {
    public:
        ...
        //! Creation routine
        void create(const multi1d<U> &u_, const CloverFermActParams &param_);
   ...
   */
        ...
        const multi1d<PrimitiveClovTriang<REALT>> &getTriBuffer() const { return tri; }
   ...
   */
    ...
    // Empty constructor. Must use create later
    template <typename T, typename U> QDPCloverTermT<T, U>::QDPCloverTermT() {}

    // Now copy
    template <typename T, typename U>
    void QDPCloverTermT<T, U>::create(const multi1d<U> &u_, const CloverFermActParams &param_,
                                      const QDPCloverTermT<T, U> &from) {

        u.resize(Nd);
        for (int mu = 0; mu < Nd; mu++) { u[mu] = u_[mu]; }
        param = param_;
        ...
        for (int i = 0; i < rb.numSubsets(); i++) { choles_done[i] = from.choles_done[i]; }
    ...
    void QDPCloverTermT<T, U>::create(const multi1d<U> &u_, const CloverFermActParams &param_) {
        ...
        choles_done.resize(rb.numSubsets());
        for (int i = 0; i < rb.numSubsets(); i++) { choles_done[i] = false; }
    }
   ...
   */
    ...
    namespace QDPCloverEnv {
        ...
        template <typename U> struct QDPCloverMakeClovArg {
   ...
   * Computes the inverse of the term on cb using Cholesky
   */
    template <typename T, typename U> void QDPCloverTermT<T, U>::choles(int cb) {
    ...
    namespace QDPCloverEnv {
        template <typename U> struct LDagDLInvArgs {
            typedef typename WordType<U>::Type_t REALT;
            typedef OScalar<PScalar<PScalar<RScalar<REALT>>>> RealT;
        ...
        inline void LDagDLInvSiteLoop(int lo, int hi, int myId, LDagDLInvArgs<U> *a) {
            ...
            for (int ssite = lo; ssite < hi; ++ssite) {
                ...
                for (int block = 0; block < 2; block++) {
                    ...
                    for (int i = 0; i < N; i++) { inv_d[i] = tri[site].diag[block][i]; }

                    for (int i = 0; i < 15; i++) { inv_offd[i] = tri[site].offd[block][i]; }
                    ...
                    for (int i = 0; i < N; i++) {
                        ...
                        // on site
                        if (inv_d[i].elem() < 0) { site_neg_logdet++; }
                    }
                    ...
                    for (int k = 0; k < N; ++k) {
                        ...
                        for (int i = 0; i < k; ++i) { zero_rep(v[i]); }
                    ...
                    for (int i = 0; i < N; i++) { tri[site].diag[block][i] = inv_d[i]; }
                    for (int i = 0; i < 15; i++) { tri[site].offd[block][i] = inv_offd[i]; }
                }
   ...
   */
    template <typename T, typename U>
    void QDPCloverTermT<T, U>::applySite(T &chi, const T &psi, int isign, int site) const {
    ...
    template <typename T, typename U>
    Real QDPCloverTermT<T, U>::getCloverCoeff(int mu, int nu) const {
    ...
    namespace QDPCloverEnv {
        ...
        template <typename T> struct ApplyArgs {
            typedef typename WordType<T>::Type_t REALT;
            T &chi;
            const T &psi;
...
#    warning "Using unrolled clover term - version 2"
   ...
   */
    template <typename T, typename U>
    void QDPCloverTermT<T, U>::apply(T &chi, const T &psi, int isign, int cb) const {
        assert(isign >= 0);
