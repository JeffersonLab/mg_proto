...
void DslashDirQDPXX(QDP::LatticeFermion& out, const QDP::multi1d<QDP::LatticeColorMatrix>& u, const QDP::LatticeFermion& in, int dir);
...
void caxpyAggrQDPXX(const std::complex<double>& alpha, const QDP::LatticeFermion& x, QDP::LatticeFermion& y, int site, int aggr);
...
std::complex<double>
innerProductAggrQDPXX(const QDP::LatticeFermion& left, const QDP::LatticeFermion& right, int site, int aggr);
...
void extractAggregateQDPXX(QDP::LatticeFermion& target, const QDP::LatticeFermion& src, int aggr );


//! 'Restrict' a QDP++ spinor to a CoarseSpinor with the same geometry (a 'block' is a 'site')
void restrictSpinorQDPXXFineToCoarse( const QDP::multi1d<QDP::LatticeFermion>& v, const QDP::LatticeFermion& ferm_in, CoarseSpinor& out);

//! 'Prolongate' a CoarseSpinor to a QDP++ Fine Spinor  ( a 'block' is a 'site'
void prolongateSpinorCoarseToQDPXXFine( const QDP::multi1d<QDP::LatticeFermion>& v, const CoarseSpinor& coarse_in, QDP::LatticeFermion& fine_out);

//! Coarsen one direction of a 'dslash' link ( 1 block = 1 site )
void dslashTripleProductSiteDirQDPXX(int dir, const QDP::multi1d<QDP::LatticeColorMatrix>& u, const QDP::multi1d<QDP::LatticeFermion>& in_vecs, CoarseGauge& u_coarse);

//! Coarsen the clover term (1 block = 1 site )
void clovTripleProductSiteQDPXX(const QDPCloverTerm& clov,const QDP::multi1d<QDP::LatticeFermion>& in_vecs, CoarseGauge& cl_coarse);

//! Coarsen one direction of a 'dslash' link, but the vector space is strictly 12x12 and is held in a propagator -- for testing
void dslashTripleProduct12x12SiteDirQDPXX(int dir, const QDP::multi1d<QDP::LatticeColorMatrix>& u, const QDP::LatticePropagator& in_prop, QDP::LatticePropagator& out_prop);
...
void clovTripleProduct12cx12SiteQDPXX(const QDPCloverTerm& clov, const QDP::LatticePropagator& in_prop, QDP::LatticePropagator& out_prop);
