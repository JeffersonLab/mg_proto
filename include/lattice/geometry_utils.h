...
namespace MG {
	 ...
	 */
    inline void AssertVectorsDivideEachOther(const IndexArray &dims_num,
    ...
    inline void AssertEquals(const IndexType v1, const IndexType v2) {
        if (v1 != v2) {
            MG::MasterLog(MG::ERROR, "IndexType values are unequal %u and %u", v1, v2);
        }
    }

    inline void IndexToCoords(unsigned int index, const IndexArray &dims, IndexArray &coords) {
    ...
    inline unsigned int CoordsToIndex(const IndexArray &coords, const IndexArray &dims) {
        int dimsize = n_dim;
    ...
    inline void IndexToCoords3(unsigned int index, const IndexArray &dims, const int mu,
                               IndexArray &coords) {
    ...
    inline void CBIndexToCoords(const int cbsite, const int cb, const IndexArray &lattice_size,
                                const IndexType cborigin, IndexArray &coords) {
    ...
    inline void CBIndexToCoords(const int cbsite, const int cb, const IndexArray &lattice_size,
                                const IndexArray &origin, IndexArray &coords) {
        CBIndexToCoords(cbsite, cb, lattice_size, origin[0] + origin[1] + origin[2] + origin[3],
                        coords);
    }

    // Coords and Dims are uncheckerboarded.
    inline void CoordsToCBIndex(const IndexArray &coords, const IndexArray &lattice_size,
                                const IndexType cborigin, int &cb, int &cbsite) {
    ...
    inline void CoordsToCBIndex(const IndexArray &coords, const IndexArray &lattice_size,
                                const IndexArray &origin, int &cb, int &cbsite) {
        CoordsToCBIndex(coords, lattice_size, origin[0] + origin[1] + origin[2] + origin[3], cb,
                        cbsite);
    }
}
