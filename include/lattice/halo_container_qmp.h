...
namespace MG {
    ...
    template <typename T> class HaloContainer {
    public:
        HaloContainer(const LatticeInfo &info)
            : _latt_info(info),
              _node_info(info.GetNodeInfo()),
              _datatype_size(haloDatumSize<T>(info)),
              _n_cols(0) {
            create(1);
        }
    ...
    private:
        void create(IndexType n_cols) {
            ...
            for (int mu = 0; mu < n_dim; ++mu) {
                ...
                if (!_local_dir[mu]) {
                    ...
                    _send_to_dir[2 * mu + MG_BACKWARD] =
                        (float *)MemoryAllocate(_face_in_bytes[mu]);
                    _send_to_dir[2 * mu + MG_FORWARD] = (float *)MemoryAllocate(_face_in_bytes[mu]);
                    _recv_from_dir[2 * mu + MG_BACKWARD] =
                        (float *)MemoryAllocate(_face_in_bytes[mu]);
                    _recv_from_dir[2 * mu + MG_FORWARD] =
                        (float *)MemoryAllocate(_face_in_bytes[mu]);

                    _msgmem_send_to_dir[2 * mu + MG_BACKWARD] =
                        QMP_declare_msgmem(_send_to_dir[2 * mu + MG_BACKWARD], _face_in_bytes[mu]);
                    _msgmem_send_to_dir[2 * mu + MG_FORWARD] =
                        QMP_declare_msgmem(_send_to_dir[2 * mu + MG_FORWARD], _face_in_bytes[mu]);
                    _msgmem_recv_from_dir[2 * mu + MG_BACKWARD] = QMP_declare_msgmem(
                        _recv_from_dir[2 * mu + MG_BACKWARD], _face_in_bytes[mu]);
                    _msgmem_recv_from_dir[2 * mu + MG_FORWARD] =
                        QMP_declare_msgmem(_recv_from_dir[2 * mu + MG_FORWARD], _face_in_bytes[mu]);

                    _mh_recv_from_dir[2 * mu + MG_BACKWARD] = QMP_declare_receive_relative(
                        _msgmem_recv_from_dir[2 * mu + MG_BACKWARD], mu, -1, 0);

                    _mh_recv_from_dir[2 * mu + MG_FORWARD] = QMP_declare_receive_relative(
                        _msgmem_recv_from_dir[2 * mu + MG_FORWARD], mu, 1, 0);

                    _mh_send_to_dir[2 * mu + MG_BACKWARD] = QMP_declare_send_relative(
                        _msgmem_send_to_dir[2 * mu + MG_BACKWARD], mu, -1, 0);

                    _mh_send_to_dir[2 * mu + MG_FORWARD] = QMP_declare_send_relative(
                        _msgmem_send_to_dir[2 * mu + MG_FORWARD], mu, 1, 0);
                }
            ...
            for (int mu = 0; mu < n_dim; ++mu) {
                ...
                if (!_local_dir[mu]) {
                    ...
                    _mh_recv_all_dir[2 * _num_nonlocal_dir + MG_BACKWARD] =
                        QMP_declare_receive_relative(_msgmem_recv_from_dir[2 * mu + MG_BACKWARD],
                                                     mu, -1, 0);

                    _mh_send_all_dir[2 * _num_nonlocal_dir + MG_FORWARD] =
                        QMP_declare_send_relative(_msgmem_send_to_dir[2 * mu + MG_FORWARD], mu, 1,
                                                  0);

                    _mh_recv_all_dir[2 * _num_nonlocal_dir + MG_FORWARD] =
                        QMP_declare_receive_relative(_msgmem_recv_from_dir[2 * mu + MG_FORWARD], mu,
                                                     1, 0);

                    _mh_send_all_dir[2 * _num_nonlocal_dir + MG_BACKWARD] =
                        QMP_declare_send_relative(_msgmem_send_to_dir[2 * mu + MG_BACKWARD], mu, -1,
                                                  0);
        ...
        void destroy() {
            ...
            // Free the combined
            if (_mh_send_all) {
                QMP_free_msghandle(_mh_send_all);
                _mh_send_all = nullptr;
            }
            if (_mh_recv_all) {
                QMP_free_msghandle(_mh_recv_all);
                _mh_recv_all = nullptr;
            }
    ...
    public:
        ...
        bool LocalDir(int d) const { return _local_dir[d]; }

        bool AmIPtMin() const { return _am_i_pt_min; }

        bool AmIPtMax() const { return _am_i_pt_max; }

        int NumNonLocalDirs() const { return _num_nonlocal_dir; }
        ...
        const LatticeInfo &GetInfo() const { return _latt_info; }

        inline size_t GetDataTypeSize() const { return _datatype_size * _n_cols; }
