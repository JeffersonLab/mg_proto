...
#    pragma simd safelen(16) simdlen(16) aligned(coarse_site_spinor, site_accum : 64)
        ...
        template <int N> class SIMDArray {
        public:
            SIMDArray() {

                for (int i = 0; i < N; i += 16) {
                    __m512 out = _mm512_set1_ps(0);
                    _mm512_store_ps(&array[i], out);
                }
            }

            float array[N] __attribute__((aligned(64)));

            inline void operator+=(const SIMDArray<N> &addme) {
                for (int i = 0; i < N; i += 16) {
                    __m512 out = _mm512_load_ps(&array[i]);
                    __m512 in = _mm512_load_ps(&addme.array[i]);
                    out = _mm512_add_ps(out, in);
                    _mm512_store_ps(&array[i], out);
                }
            }
        };
...
#    pragma omp parallel for collapse(2) schedule(static) num_threads(n_outer_threads)
            for (int block_cb = 0; block_cb < n_checkerboard; ++block_cb) {
                for (int block_cbsite = 0; block_cbsite < num_coarse_cbsites; block_cbsite++) {
                    ...
                    SIMDArray<n_floats> block_sum;

#    pragma omp declare reduction(customAdd : SIMDArray <n_floats> : omp_out += omp_in)

#    pragma omp parallel for reduction(customAdd                                                   \
                                       : block_sum) schedule(static)                               \
        num_threads(_r_threads_per_block)
                    for (IndexType fine_site_idx = 0;
                         fine_site_idx < static_cast<IndexType>(num_sites_in_block);
                         fine_site_idx++) {
                        ...
                        for (int color = 0; color < num_fine_color; ++color) {
                            ...
                            __m512 psi_lower_re =
                                _mm512_set1_ps(fine_data[RE + 2 * color + foffset]);
                            __m512 psi_lower_im =
                                _mm512_set1_ps(fine_data[IM + 2 * color + foffset]);
                            ...
                            for (int i = 0; i < 2 * num_coarse_color; i += 16) {
                                ...
                                __m512 accum_vec = _mm512_load_ps(&site_accum.array[i]);
                                ...
                                _mm512_store_ps(&site_accum.array[i], accum_vec);
                            }

                            for (int i = 2 * num_coarse_color; i < 4 * num_coarse_color; i += 16) {
                                __m512 v_vec = _mm512_load_ps(&v[i]);
                                __m512 accum_vec = _mm512_load_ps(&site_accum.array[i]);
                        ...
                        // Use array wrapper +=() prior to custom reduction
                        block_sum += site_accum;
...
#    pragma omp simd simdlen(16) safelen(16) aligned(coarse_site_spinor : 64)
                    for (int i = 0; i < n_floats; ++i) {
                        coarse_site_spinor[i] = block_sum.array[i];
                    }
