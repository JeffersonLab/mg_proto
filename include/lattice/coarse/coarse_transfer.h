                ...
                for (int block_cbsite = 0; block_cbsite < num_coarse_cbsites; ++block_cbsite) {
                    ...
                    for (IndexType fine_site_idx = 0;
                        ...
                        for (int v = 0; v < _n_vecs; ++v) {
                            ...
                            const float *vsite =
                                vectors[v]->GetSiteDataPtr(0, fine_cbsite.cb, fine_cbsite.site);
...
#pragma omp barrier
        ...
        void R_op(int num_coarse_color, const CoarseSpinor &fine_in, int source_cb,
                  CoarseSpinor &out) const {
...
#pragma omp parallel for collapse(2) schedule(static)
                ...
                for (int block_cbsite = 0; block_cbsite < num_coarse_cbsites; ++block_cbsite) {
                    ...
                    std::complex<float> *coarse_site_spinor =
                        reinterpret_cast<std::complex<float> *>(
                            out.GetSiteDataPtr(0, block_cb, block_cbsite));
                        ...
                        if (source_cb < 0 || fine_cbsite.cb == source_cb) {
                            ...
                            const std::complex<float> *fine_cbsite_data =
                                reinterpret_cast<const std::complex<float> *>(
                                    fine_in.GetSiteDataPtr(0, fine_cbsite.cb, fine_cbsite.site));
                            const std::complex<float> *v =
                                reinterpret_cast<const std::complex<float> *>(
                                    (*this).indexPtr(block_idx, fine_site_idx, 0));

                            // Do the upper chiral component
                            CMatMultCoeffAddNaive(1.0, coarse_site_spinor, num_coarse_color * 2,
                                                  1.0, v, num_coarse_color * 2, fine_cbsite_data,
                                                  num_fine_color * 2, num_coarse_color,
                                                  num_fine_color, ncol);

                            // Do the lower chiral component
                            CMatMultCoeffAddNaive(
                                1.0, &coarse_site_spinor[num_coarse_color], num_coarse_color * 2,
                                1.0, &v[num_coarse_color], num_coarse_color * 2,
                                &fine_cbsite_data[num_fine_color], num_fine_color * 2,
        ...
        void P_op(int num_coarse_color, const CoarseSpinor &coarse_in, int target_cb,
                  CoarseSpinor &fine_out) const {
...
#pragma omp parallel for collapse(2) schedule(static)
            for (int cbi = 0; cbi < ncb; ++cbi) {
                for (int fsite = 0; fsite < num_fine_cbsites; ++fsite) {
                    ...
                    std::complex<float> *fine_site_spinor = reinterpret_cast<std::complex<float> *>(
                        fine_out.GetSiteDataPtr(0, cb, fsite));
                    const std::complex<float> *coarse_site_spinor =
                        reinterpret_cast<const std::complex<float> *>(
                            coarse_in.GetSiteDataPtr(0, block_cb, block_cbsite));
                    const std::complex<float> *v = reinterpret_cast<const std::complex<float> *>(
                        (*this).indexPtr(block_idx, fine_site_idx, 0));

                    // Do the upper chiral component
                    CMatAdjMultCoeffAddNaive(1.0, fine_site_spinor, num_fine_color * 2, 1.0, v,
                                             num_coarse_color * 2, coarse_site_spinor,
                                             num_coarse_color * 2, num_coarse_color, num_fine_color,
                                             ncol);

                    // Do the lower chiral component
                    CMatAdjMultCoeffAddNaive(
                        1.0, &fine_site_spinor[num_fine_color], num_fine_color * 2, 1.0,
                        &v[num_coarse_color], num_coarse_color * 2,
                        &coarse_site_spinor[num_coarse_color], num_coarse_color * 2,
                        num_coarse_color, num_fine_color, ncol);
