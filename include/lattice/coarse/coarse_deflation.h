...
#    endif
        ...
        template <typename Spinor, typename LinOpT> struct PrimmeMatrixMatvec {
            static void fun(void *x, PRIMME_INT *ldx, void *y, PRIMME_INT *ldy, int *blockSize,
    ...
    void computeDeflation(const LatticeInfo &info, const LinOpT &M, EigsParams eigs_params,
                          std::shared_ptr<Spinor> &V, std::vector<float> &values) {
        const CBSubset &cbsubset = M.GetSubset();
        size_t nLocal = (size_t)info.GetNumColorSpins() * info.GetNumCBSites() *
                        (cbsubset.end - cbsubset.start);
        ...
        if (primme.procID == 0 && primme.printLevel > 1) primme_display_params(primme);
