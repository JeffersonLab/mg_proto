...
#    define MAX_VECS 64
    ...
    public:
        CoarseTransfer(const std::vector<Block> &blocklist,
            ...
            int num_complex = _n_blocks * _sites_per_block * num_fine_colorspin *
                              num_coarse_colorspin * n_complex;
                ...
                for (int block_cbsite = 0; block_cbsite < num_coarse_cbsites; ++block_cbsite) {
                    ...
                    for (IndexType fine_site_idx = 0;
                        ...
                        for (int v = 0; v < _n_vecs; ++v) {
                            ...
                            const float *vsite =
                                vectors[v]->GetSiteDataPtr(fine_cbsite.cb, fine_cbsite.site);
...
#pragma omp barrier
        ...
        inline float &index(int block, int blocksite, int color, int chiral, int vec, int REIM) {
            return _data[REIM +
                         n_complex * (vec + _n_vecs * (chiral + 2 * (color + num_fine_color *
                                                                                 (blocksite +
                                                                                  _sites_per_block *
                                                                                      block))))];
        }

        inline const float &index(int block, int blocksite, int color, int chiral, int vec,
                                  int REIM) const {
            return _data[REIM +
                         n_complex * (vec + _n_vecs * (chiral + 2 * (color + num_fine_color *
                                                                                 (blocksite +
        ...
        inline const float *indexPtr(int block, int blocksite, int color) const {
            return &(_data[2 * n_complex * _n_vecs *
                           (color + num_fine_color * (blocksite + _sites_per_block * block))]);
        }
                ...
                for (int block_cbsite = 0; block_cbsite < num_coarse_cbsites; ++block_cbsite) {
                    ...
                    std::complex<float> *coarse_site_spinor =
                        reinterpret_cast<std::complex<float> *>(
                            out.GetSiteDataPtr(block_cb, block_cbsite));
...
#    pragma omp simd safelen(16) simdlen(16) aligned(site_accum : 64)
                    ...
                    for (IndexType fine_site_idx = 0;
                        ...
                        const std::complex<float> *fine_cbsite_data =
                            reinterpret_cast<const std::complex<float> *>(
                                fine_in.GetSiteDataPtr(fine_cbsite.cb, fine_cbsite.site));
                        ...
                        for (int f_color = 0; f_color < num_fine_color; ++f_color) {
                            ...
                            std::complex<float> psi_lower(
                                fine_cbsite_data[f_color + num_fine_color]);

                            const std::complex<float> *v =
                                reinterpret_cast<const std::complex<float> *>(
                                    (*this).indexPtr(block_idx, fine_site_idx, f_color));
...
#    pragma simd safelen(16) simdlen(16) aligned(coarse_site_spinor, site_accum : 64)
        ...
        void R_op(const CoarseSpinor &fine_in, CoarseSpinor &out) const {
            ...
            const int foffset = 2 * fine_in.GetNumColor();
            const int coffset = 2 * num_coarse_color;
            int n_outer_threads = _n_threads / _r_threads_per_block;

#    pragma omp parallel for collapse(2) schedule(static) num_threads(n_outer_threads)
            for (int block_cb = 0; block_cb < n_checkerboard; ++block_cb) {
                for (int block_cbsite = 0; block_cbsite < num_coarse_cbsites; block_cbsite++) {
                    int block_idx = block_cbsite + num_coarse_cbsites * block_cb;

                    const Block &block = _blocklist[block_idx];
                    auto block_sitelist = block.getCBSiteList();
                    auto num_sites_in_block = block_sitelist.size();

                    // One result per block: zero it before entering inner region
                    float *coarse_site_spinor = out.GetSiteDataPtr(block_cb, block_cbsite);
#    pragma omp simd simdlen(16) safelen(16) aligned(coarse_site_spinor : 64)
                    for (int i = 0; i < n_floats; ++i) { coarse_site_spinor[i] = 0; }

                    // Inner parallel region
#pragma omp parallel for reduction(+: coarse_site_spinor[:4*num_coarse_color]) num_threads(_r_threads_per_block)
                    for (IndexType fine_site_idx = 0;
                         fine_site_idx < static_cast<IndexType>(num_sites_in_block);
                         fine_site_idx++) {
                        ...
                        // Get the fine Data
                        const CBSite &fine_cbsite = block_sitelist[fine_site_idx];
                        const float *fine_data =
                            fine_in.GetSiteDataPtr(fine_cbsite.cb, fine_cbsite.site);

                        // Site local copy
                        float site_accum[n_floats] __attribute__((aligned(64)));
#    pragma omp simd safelen(16) simdlen(16) aligned(site_accum : 64)
                        for (int i = 0; i < n_floats; ++i) { site_accum[i] = 0; }

                        // Loop through fine colors
                        for (int color = 0; color < num_fine_color; ++color) {
                            float psi_upper_re = fine_data[RE + 2 * color];
                            float psi_upper_im = fine_data[IM + 2 * color];
                            float psi_lower_re = fine_data[RE + 2 * color + foffset];
                            float psi_lower_im = fine_data[IM + 2 * color + foffset];

                            // get the vector
                            const float *v = ((*this).indexPtr(block_idx, fine_site_idx, color));

#    pragma omp simd simdlen(16) safelen(16) aligned(v, site_accum : 64)
                            for (int i = 0; i < 2 * num_coarse_color; i += 2) {

                                float v_re = v[RE + i];
                                float v_im = v[IM + i];

                                // (v_re, -v_im) * (psi_upper_re, psi_upper_im)
                                site_accum[RE + i] += v_re * psi_upper_re - v_im * psi_upper_im;
                                site_accum[IM + i] += v_re * psi_upper_im + v_im * psi_upper_re;
                            }

#    pragma omp simd simdlen(16) safelen(16) aligned(v, site_accum : 64)
                            for (int i = 2 * num_coarse_color; i < 4 * num_coarse_color; i += 2) {
                                float v_re = v[RE + i];
                                float v_im = v[IM + i];

                                // (v_re, -v_im) * (psi_upper_re, psi_upper_im)
                                site_accum[RE + i] += v_re * psi_lower_re - v_im * psi_lower_im;
                                site_accum[IM + i] += v_re * psi_lower_im + v_im * psi_lower_re;
                            }
                        }

#    pragma omp simd simdlen(16) safelen(16) aligned(coarse_site_spinor, site_accum : 64)
...
#    pragma omp parallel for collapse(2)
            for (int cb = 0; cb < n_checkerboard; ++cb) {
                for (int fsite = 0; fsite < num_fine_cbsites; ++fsite) {
                    ...
                    std::complex<float> *fine_site_data =
                        reinterpret_cast<std::complex<float> *>(fine_out.GetSiteDataPtr(cb, fsite));
                    const std::complex<float> *coarse_site_spinor =
                        reinterpret_cast<const std::complex<float> *>(
                            coarse_in.GetSiteDataPtr(block_cb, block_cbsite));
                    ...
                    for (int fcolor = 0; fcolor < num_fine_color; ++fcolor) {
                        ...
                        const std::complex<float> *v =
                            reinterpret_cast<const std::complex<float> *>(
                                (*this).indexPtr(block_idx, fine_site_idx, fcolor));
...
#    pragma omp simd safelen(8) simdlen(16) aligned(v, coarse_site_spinor : 64)
                        for (int i = 0; i < num_coarse_color; ++i) {
                            reduce_lower += conj(v[i + num_coarse_color]) *
                                            coarse_site_spinor[i + num_coarse_color];
                        }
...
#    pragma omp simd safelen(16) simdlen(16) aligned(vec_re : 64)
                        for (int i = 0; i < 16; ++i) { reduce_upper_re += vec_re[i]; }

#    pragma omp simd safelen(16) simdlen(16) aligned(vec_im : 64)
                        for (int i = 0; i < 16; ++i) { reduce_upper_im += vec_im[i]; }
                        ...
                        for (int i = 0; i < 2 * num_coarse_color; i += 16) {
                            ...
                            sum_r_vec = _mm512_fmadd_ps(v_vec, c_vec, sum_r_vec);
                            sum_i_vec = _mm512_fmadd_ps(
                                v_vec, _mm512_shuffle_ps(c_vec, c_vec, 0xb1), sum_i_vec);
                        }
                        sum_i_vec = _mm512_mul_ps(sum_i_vec, sign);
                        _mm512_store_ps(vec_re, sum_r_vec);
                        _mm512_store_ps(vec_im, sum_i_vec);
...
#    pragma omp simd safelen(16) simdlen(16) aligned(vec_re : 64)
                        for (int i = 0; i < 16; ++i) { reduce_lower_re += vec_re[i]; }

#    pragma omp simd safelen(16) simdlen(16) aligned(vec_im : 64)
                        for (int i = 0; i < 16; ++i) { reduce_lower_im += vec_im[i]; }
...
#    pragma simd safelen(16) simdlen(16) aligned(fine_site_tmp, fine_site_data : 64)
        ...
        ~CoarseTransfer() { MemoryFree(_data); }
