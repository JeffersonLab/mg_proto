...
namespace MG {
	...
	public:
		...
		/** GetSiteChiral component
		 *
		 *  Returns a pointer to the correct chiral component
		 */
		inline
		float *GetSiteChiralDataPtr(IndexType cb, IndexType site, IndexType chiral)
		{
			return &data[cb][site*_n_site_offset + chiral*_n_block_offset];
		}

		inline
		const float *GetSiteChiralDataPtr(IndexType cb, IndexType site, IndexType chiral) const
				{
					return &data[cb][site*_n_site_offset + chiral*_n_block_offset];
				}
