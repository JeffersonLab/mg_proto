...
namespace MGGeometry {
	...
	template<typename BlockedArrayOut, typename SpinorIn>
	void zip(BlockedArrayOut& v_out, const SpinorIn& spinor_in)
	{
		...
		IndexType num_dest_colors = info.GetNumColors();
		...
		for (int block = 0; block < num_blocks; ++block) {
			for (int dst_spin = 0; dst_spin < num_dest_spins; dst_spin++) {
				...
				for (IndexType s_spin = 0; s_spin < n_spin; ++s_spin) {
					...
					for (int s_color = 0; s_color < n_color; s_color++) {
