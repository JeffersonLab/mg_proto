...
TEST(TestKokkos, TestSpinProject)
{
	...
	KokkosCBFineSpinor<MyComplex<REAL32>,4> kokkos_in(info,EVEN);
	KokkosCBFineSpinor<MyComplex<REAL32>,2> kokkos_hspinor_out(hinfo,EVEN);
...

	{
	  ...
	  for(int i=0; i < iters; ++i) {
	    KokkosProjectLattice<MyComplex<REAL32>,0,-1>(kokkos_in,kokkos_hspinor_out);
	  }
	...
	}
	{
	  ...
	  double start_time = omp_get_wtime();
	  for(int i=0; i < iters; ++i) {
	    KokkosProjectLattice<MyComplex<REAL32>,1,-1>(kokkos_in,kokkos_hspinor_out);
	  }
	  double end_time = omp_get_wtime();
 	  double effective_bw = mem_moved_in_GB / (end_time-start_time);
	  MasterLog(INFO, "Effective Bandwidth = %16.8e GB/sec\n", effective_bw);
	}
	{
	  ...
	  double start_time = omp_get_wtime();
	  for(int i=0; i < iters; ++i) {
	    KokkosProjectLattice<MyComplex<REAL32>,2,-1>(kokkos_in,kokkos_hspinor_out);
	  }
	  double end_time = omp_get_wtime();
 	  double effective_bw = mem_moved_in_GB / (end_time-start_time);
	  MasterLog(INFO, "Effective Bandwidth = %16.8e GB/sec\n", effective_bw);
	}
	{
	  ...
	  double start_time = omp_get_wtime();
	  for(int i=0; i < iters; ++i) {
	    KokkosProjectLattice<MyComplex<REAL32>,3,-1>(kokkos_in,kokkos_hspinor_out);
	  }
	  double end_time = omp_get_wtime();
 	  double effective_bw = mem_moved_in_GB / (end_time-start_time);
	  MasterLog(INFO, "Effective Bandwidth = %16.8e GB/sec\n", effective_bw);
...
TEST(TestKokkos, TestDslash)
{
  IndexArray latdims={{32,32,32,32}};
	...
	initQDPXXLattice(latdims);
	LatticeInfo info(latdims,4,3,NodeInfo());
	KokkosFineGaugeField<MyComplex<REAL32>>  kokkos_gauge(info);
	...
	KokkosCBFineSpinor<MyComplex<REAL32>,4> kokkos_spinor_in(info,EVEN);
	KokkosCBFineSpinor<MyComplex<REAL32>,4> kokkos_spinor_out(info,ODD);
	{
	...
	KokkosDslash<MyComplex<REAL32>,MyComplex<REAL32>> D(info);
...
TEST(TestKokkos, TestDslashVec)
{
  IndexArray latdims={{16,16,16,32}};
	...
	for(int per_team=2; per_team < 8192; per_team *= 2) {
	  ...
	  KokkosDslash<MyComplex<REAL32>,SIMDComplex<REAL32,V>> D(info,per_team);
#else
	  int per_team =-1;
	  KokkosDslash<MyComplex<REAL32>,SIMDComplex<REAL32,V>> D(info);
#endif
	for(int rep=0; rep < 2; ++rep) {
	  for(int isign=-1; isign < 2; isign+=2) {
