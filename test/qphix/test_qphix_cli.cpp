...
TEST(QPhiXIntegration, TestQPhiXBiCGStab)
{
  ...
  // Now let us prepare the source for QPhiX
  {
    ...
    solver(soln,rhs, 1.0e-7, n_iters, rsd_sq_final,site_flops,mv_apps,1,false, ODD);
...
TEST(QPhiXIntegration, TestQPhiXBiCGStabAbsolute)
{
  ...
  // Now let us prepare the source for QPhiX
  {
    ...
    // Skip source prep - since source is zero
    // Trans source is also zero.
    ...
    solver(soln,rhs, 1.0e-7, n_iters, rsd_sq_final,site_flops,mv_apps,1, true, ODD, QPhiX::ABSOLUTE);
    ...
    //
    {
      ...
      // Fill t1 even
      D_full.M_eo(t1,solution, LINOP_OP);
      D_full.M_ee_inv(solution,t1,LINOP_OP);
      solution[rb[EVEN]] *= -1;
    }
  }
